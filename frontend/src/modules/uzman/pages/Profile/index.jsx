...
import { 
  ...
  FileText, Calendar, ShieldCheck, ExternalLink, Activity, User, ArrowRight
} from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <p className="text-slate-500 font-bold text-xs uppercase tracking-widest">Profil Hazırlanıyor...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter italic">Profilim</h1>
          <p className="text-slate-500 font-bold text-sm uppercase tracking-widest">Platformdaki Görünümünüz</p>
        </div>
        <Button 
          ...
          <div className="flex-1 text-center lg:text-left space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3 md:gap-6">
              {[
                ...
                { icon: <FileText size={18}/>, val: profile.makaleler?.length || profile.totalArticles || 0, label: 'Makale', color: 'text-indigo-600' },
              ].map((stat, i) => (
                <div key={i} className="bg-slate-50 border border-slate-100 p-4 rounded-3xl group hover:bg-white hover:shadow-xl hover:shadow-slate-100 transition-all">
        ...
        <div className="lg:col-span-7 space-y-8">
            ...
            >
                <h2 className="text-xl font-black text-slate-900 mb-6 uppercase tracking-tighter flex items-center gap-3">
                    <Activity className="text-blue-600" size={20}/> Hakkımda
                </h2>
            ...
            >
                ...
                <Award className="text-blue-600" size={20} /> Uzmanlık Alanlarım
                </h2>
                ...
                {Object.entries(profile.uzmanlikAlanlari).map(([kategori, alanlar]) => (
                    <div key={kategori} className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <span
                                key={idx}
                                className="px-4 py-2 bg-blue-50 text-blue-700 rounded-2xl text-xs font-black border border-blue-100 shadow-sm hover:bg-blue-600 hover:text-white transition-colors cursor-default"
                            >
                                {alan}
                            </span>
                            ))}
            ...
            {profile.sertifikalar && profile.sertifikalar.length > 0 && (
                ...
                >
                    ...
                    <div className="space-y-4">
                        ...
                        <div key={cert.id} className="group p-5 bg-slate-800/50 border border-slate-700/50 rounded-3xl flex items-center justify-between hover:bg-slate-800 transition-all">
                            <div className="flex items-center gap-4">
                                <div className="p-2 bg-blue-500/20 text-blue-400 rounded-lg"><Award size={20}/></div>
